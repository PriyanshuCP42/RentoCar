...
const ManageCars = () => {
    ...
    const toggleAvailbility=async(carId)=>{
        try {
    ...
    try {
        ...
        if (data.success) {
            toast.success(data.message);
            fetchOwnerCars(); // update ManageCars page
            fetchCars();      // update homepage Featured cars
        } else {
            toast.error(data.message);
        }
                ...
                {cars.map((car,index)=>(
                    <tr key={index} className='border-t border-borderColor'>
                        ...
                        <td className='flex items-center p-3'>
                                <img onClick={()=>toggleAvailbility(car._id)} src={car.isAvaliable ? assets.eye_close_icon:assets.eye_icon} alt="" className='cursor-pointer'/>
