...
import { assets, cityList } from '../assets/asset'
    ...
    return (
        <motion.div
            ...
            <motion.h1
                ...
                className='text-4xl md:text-5xl font-semibold'>Luxury Car on Rent</motion.h1>
            <motion.form
                initial={{ scale: 0.95, opacity: 0, y: 50 }}
                animate={{ scale: 1, opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: 0.4 }}
                onSubmit={handleSerach} className='flex flex-col md:flex-row items-start md:tems-center justify-between p-6 
                    rounded-lg md:rounded-full w-full max-w-80 md:max-w-200 bg-white shadow-[0px_8px_20px_rgba[(0,0,0,0.1)]'>

                <div className='flex flex-col md:flex-row items-start md:items-center gap-10 md:ml-8'>
                    <div className='flex flex-col items-start gap-2'>
                        <select required value={pickupLoaction} onChange={(e) => setPickupLocation(e.target.value)}>
            ...
            <motion.img
                ...
                src={assets.main_car} alt="car-img" className='max-h-74' />
        </motion.div>
    )
}
